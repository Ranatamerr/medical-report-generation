...
def parse_agrs():
    ...
    parser.add_argument('--max_train_samples', type=int, default=None, help='limit training set size (e.g. 5000).')
    ...
    parser.add_argument('--record_dir', type=str, default='records_acl/', help='the patch to save the results of experiments.')
...
def main():
    ...
    ve_optimizer, ed_optimizer = build_noamopt_optimizer(args, model)
    ...
    # Copy results to Google Drive if running on Colab
    drive_dir = '/content/drive/MyDrive/Bachelor/results'
    if os.path.exists('/content/drive'):
        os.makedirs(drive_dir, exist_ok=True)
        # Copy checkpoints
        if os.path.exists(args.save_dir + '_seed_' + str(args.seed)):
            shutil.copytree(
                args.save_dir + '_seed_' + str(args.seed),
                os.path.join(drive_dir, os.path.basename(args.save_dir) + '_seed_' + str(args.seed)),
                dirs_exist_ok=True
            )
        # Copy records CSV
